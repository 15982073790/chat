...
class MeiLiBo_Sniffs_Commenting_FileCommentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    /**
     * Author tag must be 'Xxx Yyyy<yyyx@wolive.cc>'.
     *
     * @param int $errorPos The line number where the error occurs.
     *
     * @return void
    ...
    protected function processAuthors($errorPos)
    {
        ...
        if (empty($authors) === false) {
            ...
            } else if (!preg_match('%^.*\<[a-z0-9]+\@wolive\.cc\>$%', $content)) {
                $error = 'Expected "Xxx Yyy<yyyx@wolive.cc>" for author tag';
