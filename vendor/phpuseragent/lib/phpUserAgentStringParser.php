...
class phpUserAgentStringParser
{
  ...
  protected function doParse($userAgentString)
  {
    ...
    if (preg_match_all($pattern, $userAgent['string'], $matches))
    {
      ...
      {
          if(strpos($userAgent['string'],'micromessenger')!==false){
              $userAgent['browser_name']='micromessenger';
          }elseif(strpos($userAgent['string'],'ubrowser')!==false){
              $userAgent['browser_name']='ubrowser';
          }elseif(strpos($userAgent['string'],'lbbrowser')!==false){
              $userAgent['browser_name']='lbbrowser';
          }elseif(strpos($userAgent['string'],'se 2.x')!==false){
              $userAgent['browser_name']='sogoubrowser';
          }elseif(strpos($userAgent['string'],'qq')!==false){
              if(strpos($userAgent['string'],'_sq_')!==false){
                  $userAgent['browser_name']='qqnbrowser';
              }else{
                  $userAgent['browser_name']='qqbrowser';
              }
    ...
    if (preg_match($pattern, $userAgent['string'], $match))
    {
      if (isset($match[0]))
      {
          if(strpos($userAgent['string'],'android')!==false){
