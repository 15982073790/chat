<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:90:"/data/wwwroot/dev/chat.profittravel.com/public/../application/admin/view/custom/index.html";i:1619138108;s:81:"/data/wwwroot/dev/chat.profittravel.com/application/admin/view/public/header.html";i:1620746200;s:82:"/data/wwwroot/dev/chat.profittravel.com/application/admin/view/public/footer2.html";i:1613705904;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
...
<div id="layout-center">
    ...
    <script src="/assets/libs/vue/vue.js?v=LK_DIY6.0.3" type="text/javascript"></script>
    <style>    #container {
            display: flex;
    ...
    <div id="container" style="overflow-y: auto">
        <div id="group_list" style="display: none;">
            <div class="checkbox-list">
                <div class="checkbox-item" v-for="item in group"><input class="checkbox" v-model="item.checked"
                                                                        name="group" :value="item.id"
                                                                        type="checkbox"><span
                            style="margin-left: 15px;">{{item.group_name}}</span></div>
                <p class="addgroup" style="margin-top: 10px;" onclick="createdGroup()">+ 添加分组</p>
                <div class="addgroup-input input-group" style="margin-bottom: 25px"><input type="text"
                                                                                           placeholder="请输入分组名，最多12个字"
                                                                                           class="form-control"><span
                            class="input-group-btn"><button class="btn btn-primary" style="background-color:#2E9FFF"
                                                            type="button"
                                                            onclick="add()">添加</button></span></div>
            </div>
        </div>
        <div class="left"><p class="left-title">客户分组</p>
            <ul style="max-height: 400px;overflow: auto">
                <li :class="group_id == 0 ? 'active' : ''" @click="ajax(0,1,'全部客户')"><span>全部客户</span>
                    <div class="number" style="display: block;"><?php echo $allcount; ?></div>
                </li>
                <li :class="group_id == -2 ? 'active' : ''" @click="ajax(-2,1,'意向客户')"><span>意向客户</span>
                    <div class="number" style="display: block;"><?php echo $message_count; ?></div>
                </li>
                <li class="group-item" v-for="item in group" :class="group_id == item.id ? 'active' : ''"
                    @click="ajax(item.id,1,item.group_name)"><span>{{item.group_name}}</span>
                    <div class="number">{{item.count}}</div>
                    <img class="more" @click.stop="show(item.id,item.group_name,item.bgcolor,$event)"
                         src="/assets/images/admin/more.png" alt=""></li>
                <div class="more-menu left-menu" v-if="toggle" :style="{top: top}">
                    <div @click.stop="editgroup" class="menu-item">修改名称</div>
                    <div @click.stop="delGroup" class="menu-item">删除分组</div>
                </div>
            </ul>
            <p class="addgroup" style="margin-left: 15px;" onclick="addgroup()">+ 添加分组</p>
            <ul>
                <li @click="ajax(-1,1,'黑名单')"><span>黑名单</span>
                    <div class="number" style="display: block;"><?php echo $blackcount; ?></div>
                </li>
            </ul>
        </div>
        <div class="right"><p class="right-title">{{title}}</p>
            <div class="search" v-if="operate == false"><input class="search-input" v-model="keyword" type="text"
                                                               placeholder="昵称或姓名" @keyup.enter="search"><img
                        class="search-icon" @click.stop="search" src="/assets/images/admin/icon-search.png"></div>
            <div class="choose" @click.stop="toEdit" v-if="!operate">
                <button class="choose-btn">批量操作</button>
            </div>
            <div class="operate-menu" v-if="operate == true">
                <button type="operate-btn" @click="setGroup">添加到
                    <span class="caret"></span></button>
                <button class="operate-btn" @click.stop="moreblack">加入黑名单</button>
                <button class="operate-btn" @click.stop="toEdit">退出管理</button>
            </div>
            <div id="wrap" class="wrap">
                <div @click.stop="selecte(item)" class="item" :class="{hand:title != '黑名单',selected:item.choose}"
                     v-for="item in list" :key="item.vid">
                    <div v-if="item.group_name_array.length > 1 && show_id == item.vid"
                         class="triangle_border_up"></div>
                    <span v-if="item.group_name_array.length > 1 && show_id == item.vid" class="group-name-array">{{item.group_name}}</span><img
                            v-if="item.choose" class="wrap-status" src="/assets/images/admin/choose.png" alt=""><img
                            :src="item.avatar" alt="" :class="item.state==='offline'?'wrap-img icon_gray':'wrap-img'">
                    <div class="wrap-name"><span v-if="item.name || item.tel" class="tag"><span>已留信息</span><div
                                    class="msg-info"><div>{{item.name}}</div><div>{{item.tel}}</div><div
                                        style="color: #999">{{item.msg_time}}</div><div
                                        class="border"></div></div></span><span>{{item.visiter_name}}</span></div>
                    <div class="group-name" v-if="item.group_name && title != '黑名单'"><span :data-id="item.vid"
                                                                                           class="group-name-item group-first-name">{{item.group_name_array[0]}}</span><span
                                v-if="item.group_name_array.length > 1">...</span></div>
                    <div class="group-name" v-if="title == '黑名单'"><span class="group-name-item"
                                                                        @click.stop="remove(item.visiter_id)">移出黑名单</span>
                    </div>
                    <img v-if="title != '黑名单'" class="more" @click.stop="showMenu(item)"
                         src="/assets/images/admin/more.png"
                         alt="">
                    <div class="more-menu" style="top: 80px" v-if="vid == item.vid && title != '黑名单'">
                        <div class="menu-item" @click.stop="setGroup">修改分组</div>
                        <div class="menu-item" @click.stop="toBlack(item.visiter_id)">加入黑名单</div>
                    </div>
                </div>
            </div>
            <div class="page">
                <ul class="pagination" v-if="list.length > 0">
                    <li @click="lastPage" class="page-item" :class="{disabled:page.current_page == 1}"><a
                                onclick="return false">&laquo;</a></li>
                    <li v-if="length > 1" @click="choosePage" class="page-item"
                        :class="{active:page.current_page == 1}"><a :data-id="1" onclick="return false">1</a></li>
                    <li v-if="left_more" class="page-item disabled"><a>…</a></li>
                    <li @click="choosePage" v-for="item in page_list" class="page-item"
                        :class="{active:page.current_page == item}"><a :data-id="item"
                                                                       onclick="return false">{{item}}</a></li>
                    <li v-if="right_more" class="page-item disabled"><a>…</a></li>
                    <li @click="choosePage" class="page-item" :class="{active:page.current_page == length}"><a
                                :data-id="length" onclick="return false">{{length}}</a></li>
                    <li @click="nextPage" class="page-item" :class="{disabled:page.current_page == length}"><a
                                onclick="return false">&raquo;</a></li>
                </ul>
            </div>
        </div>
    </div>
...
<div style="display: none;" id="addgroup">
    <form class="layui-form" style="margin-top:30px;padding: 0 40px;">
        <div class="layui-form-item"><label class="layui-form-label"
                                            style="font-size: 13px;color: #999999;">分组名:</label>
            <div class="layui-input-block"><input type="text" id="classname" required="" placeholder="请输入分组名，最多12个字"
                                                  autocomplete="off" class="layui-input" style="width:260px;"></div>
        </div>
        <div class="layui-form-item"><label class="layui-form-label">标签背景色</label>
            <div class="layui-input-block">
                <div class="layui-input-inline"><input type="hidden" name="bgcolor" id="bgcolor" placeholder="请选择颜色"
                                                       class="layui-input"
                                                       value="#707070"></div>
...
<script type="text/javascript">    var AddgroupDom=$('#addgroup'),colorpicker,bgcolorDom=$('#bgcolor'),classnameDom=$("#classname");
